...
import { ArrowLeft, Trash2, Save, Plus, X, Search, Edit2, FileText, Copy, Check, AlertCircle, History, Calendar, Clock, GraduationCap, BookOpen, Target, Languages, TrendingUp, Zap, RotateCcw, Info, Type as TypeIcon, AlignLeft, Keyboard, ArrowRightLeft, ChevronDown, ChevronUp, Shuffle, BarChart3, GripVertical, Dices, Settings, ListOrdered, Scale, AlignJustify } from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';
import { getDynamicColor, getBadgeColor, countWords } from '../App';
...
export const DeckEditor: React.FC<DeckEditorProps> = ({ deck, onUpdateDeck, onBack }) => {
  ...
  const [sortTwMin, setSortTwMin] = useState<string>('');
  const [sortTwMax, setSortTwMax] = useState<string>('');
  ...
  // New: Local state for stats options
  ...
  const displayPhrases = useMemo(() => {
    ...
    // Sort by Queue Order to satisfy "Shuffle then follow order"
    return filtered.sort((a, b) => {
        const idxA = deck.queue.indexOf(a.id);
        const idxB = deck.queue.indexOf(b.id);
        // If not found in queue (deleted/bug), push to end
        if (idxA === -1) return 1;
        if (idxB === -1) return -1;
        return idxA - idxB;
    });
  ...
  const stats = useMemo(() => {
      ...
      let perfectCount = 0;
      phrases.forEach(p => {
          const weight = subject === 'English' ? countWords(p.english) : 1;
          totalW += weight;
          // Use 0 if totalWrong is undefined, or handle negative legacy values by max(0, val)
          const tw = Math.max(0, p.totalWrong ?? (p.totalReviews - (p.consecutiveCorrect > 0 ? p.consecutiveCorrect : 0)));
          
          // Strict Perfect condition: Total Wrong is 0 AND Current Streak is not Wrong (consecutiveWrong == 0)
          // Also require at least 1 review to be "perfect" (not just new)
          const isPerfect = tw === 0 && p.consecutiveWrong === 0 && p.totalReviews > 0;
      ...
      return { totalW, perfW, perfectCount };
  }, [phrases, subject]);
  ...
  const availableTags = useMemo(() => {
      ...
      tags.add('新');
      deck.phrases.forEach(p => { tags.add(getTag(p)); });
  ...
  const generateBatchText = () => {
    ...
    return sortedIds.map((id, idx) => {
      ...
      const progress = p.consecutiveCorrect > 0 ? p.consecutiveCorrect : -p.consecutiveWrong;
      // Using ?? to strictly access totalWrong
      const totalWrong = p.totalWrong ?? 0;
      return `${p.chinese} | ${p.english} | ${note} | ${progress} | ${idx} | ${totalWrong}`;
    }).filter(x => x).join('\n');
  };

  const handleResetProgress = () => {
    if (resetInput !== 'RESET') return;
    const resetPhrases = phrases.map(p => ({ ...p, consecutiveCorrect: 0, consecutiveWrong: 0, mastery: 0, totalReviews: 0, totalWrong: 0, lastReviewedAt: undefined }));
    // Restore original order (serial number order)
    const orderedQueue = resetPhrases.map(p => p.id);
    const updatedDeck: Deck = { ...deck, name: deckName, subject, phrases: resetPhrases, queue: orderedQueue, stats: { totalStudyTimeSeconds: 0, totalReviewCount: 0 }, sessionHistory: [] };
  ...
  const handleSmartSort = () => {
      ...
      const twMinVal = sortTwMin === '' ? -1 : parseInt(sortTwMin);
      const twMaxVal = sortTwMax === '' ? Infinity : parseInt(sortTwMax);
      ...
      allIds.forEach(id => {
          ...
          // Selection Logic: Tag match OR TW Range match
          const tagMatch = selectedTags.has(tag);
          const twMatch = (sortTwMin !== '' || sortTwMax !== '') ? (tw >= twMinVal && tw <= twMaxVal) : false;
      ...
      if (sortStrategy === 'head') {
          ...
          // Interleave 1:X ratio
          const ratio = Math.max(1, interleaveRatio);
          let tIdx = 0;
          for (let oIdx = 0; oIdx < otherIds.length; oIdx++) {
              newQueue.push(otherIds[oIdx]);
              // Insert 1 target item every `ratio` other items
              if ((oIdx + 1) % ratio === 0 && tIdx < targetIds.length) {
                  newQueue.push(targetIds[tIdx++]);
              }
          }
          // Append remaining target items
          while (tIdx < targetIds.length) { newQueue.push(targetIds[tIdx++]); }
          // Append remaining other items if any (shouldn't be, loop covers all others)
      }
      onUpdateDeck({ ...deck, queue: newQueue });
  ...
  const confirmBatchSave = () => {
    ...
    lines.forEach((line) => {
      ...
      const chi = parts[0], eng = parts[1], noteRaw = parts[2] || '', progRaw = parts[3] || '', idxRaw = parts[4] || '', totalWrongRaw = parts[5] || '';
      const note = noteRaw.replace(/\\n/g, '\\n');
      const progress = parseInt(progRaw) || 0;
      
      // Parse totalWrong explicitly
      let totalWrong = 0;
      if (totalWrongRaw !== '') {
          totalWrong = parseInt(totalWrongRaw);
          if (isNaN(totalWrong)) totalWrong = 0;
      } else {
          // If not provided in format, infer from progress if negative, else default to 0 (assume reset/perfect if unseen)
          if (progress < 0) totalWrong = Math.abs(progress);
      }
      ...
      const p: Phrase = {
        ...
        chinese: chi,
        note,
        consecutiveCorrect: progress > 0 ? progress : 0,
        consecutiveWrong: progress < 0 ? Math.abs(progress) : 0,
        mastery: existing?.mastery || 0,
        // If we have totalWrong explicitly, don't trust totalReviews derivation unless totalReviews is also imported (which it isn't).
        // So we minimally ensure totalReviews >= correct + wrong
        totalReviews: (progress > 0 ? progress : Math.abs(progress)) + totalWrong, 
  ...
  const startEditing = (p: Phrase) => {
    ...
    setEditForm({ english: p.english, chinese: p.chinese, note: (p.note || '').replace(/\\n/g, '\n') });
  };

  const savePhrase = () => {
    const updated = phrases.map(p => {
      if (p.id === editingPhraseId) return { ...p, english: editForm.english, chinese: editForm.chinese, note: editForm.note };
      return p;
    });
    setPhrases(updated);
  ...
  const formatDuration = (s: number) => { const m = Math.floor(s / 60); const rs = s % 60; return `${m}m${rs}s`; };
  const formatSmartTime = (s: number) => { if (s < 60) return `${s}s`; if (s < 3600) return `${Math.floor(s/60)}m`; return `${(s/3600).toFixed(1)}h`; };
  const formatDate = (ts: number) => { return new Date(ts).toLocaleString('zh-CN', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }); };
    ...
    return (
      <div className="mt-4 pt-4 border-t border-slate-100">
         ...
         <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible bg-slate-50/50 rounded-xl border border-slate-100">
            ...
            <line x1={padding.left} y1={padding.top + chartHeight * 0.25} x2={width-padding.right} y2={padding.top + chartHeight * 0.25} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="4 2" />
            <line x1={padding.left} y1={padding.top + chartHeight * 0.75} x2={width-padding.right} y2={padding.top + chartHeight * 0.75} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="4 2" />
  ...
  const renderMacroChart = () => {
      ...
      let allPoints: {t: number, v: number}[] = [];
      allPoints.push({t: 0, v: 0});
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="w-full">
                <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible">
                    ...
                    <line x1={padding.left + chartWidth * 0.25} y1={padding.top} x2={padding.left + chartWidth * 0.25} y2={height-padding.bottom} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="3 3" />
                    <line x1={padding.left + chartWidth * 0.50} y1={padding.top} x2={padding.left + chartWidth * 0.50} y2={height-padding.bottom} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="3 3" />
                    <line x1={padding.left + chartWidth * 0.75} y1={padding.top} x2={padding.left + chartWidth * 0.75} y2={height-padding.bottom} stroke="#e2e8f0" strokeWidth="1" strokeDasharray="3 3" />
  ...
  const renderLogCard = (log: DeckSessionLog) => {
    ...
    const modeLabel = log.mode === 'STUDY' ? '背诵' : '考试';
    const accuracy = log.reviewCount > 0 ? (log.correctCount / log.reviewCount) * 100 : 0;
    const gain = log.masteryGain !== undefined ? log.masteryGain : (log.masteryEnd !== undefined && log.masteryStart !== undefined ? log.masteryEnd - log.masteryStart : undefined);

    return (
      <div key={log.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm hover:border-indigo-200 transition-all">
        <div className="flex justify-between items-start cursor-pointer" onClick={() => setExpandedLogId(isExpanded ? null : log.id)}>
           <div>
              <div className="flex items-center gap-2 mb-1">
                 <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${log.mode === 'STUDY' ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}>{modeLabel}</span>
                 <span className="text-xs font-bold text-slate-400">{dateStr}</span>
              </div>
              <div className="flex items-center gap-3 text-sm font-bold text-slate-700">
                 <span>{durationStr}</span>
                 <span className="text-slate-300">|</span>
                 <span className="text-emerald-600">{log.correctCount} 对</span>
                 <span className="text-rose-500">{log.wrongCount} 错</span>
              </div>
           </div>
           <div className="text-right">
              <div className="text-2xl font-black text-slate-800" style={{ color: getDynamicColor(accuracy) }}>{accuracy.toFixed(2)}%</div>
              {gain !== undefined && (
                <div className="flex flex-col items-end">
                  <div className={`text-[10px] font-black ${gain > 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                     {gain > 0 ? '+' : ''}{gain.toFixed(2)}%
                  </div>
                  {(log.masteryStart !== undefined && log.masteryEnd !== undefined) && (
                       <div className="text-[9px] text-slate-400 font-bold mt-0.5 tabular-nums">
                           {log.masteryStart.toFixed(2)}% <span className="text-slate-300">→</span> {log.masteryEnd.toFixed(2)}%
                       </div>
                  )}
                </div>
              )}
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-slate-50 animate-in fade-in slide-in-from-top-2">
             {log.mode === 'EXAM' && log.examResults && (
                <div className="mb-4 space-y-2">
                   <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Wrong Answers</div>
                   {log.examResults.filter(r => !r.isCorrect).map((r, i) => (
                      <div key={i} className="flex justify-between items-center text-xs bg-rose-50 p-2 rounded-lg text-rose-800">
                         <span>{r.q}</span>
                         <span className="font-bold">{r.a}</span>
                      </div>
                   ))}
                   {log.examResults.every(r => r.isCorrect) && <div className="text-xs text-emerald-600 font-bold italic">全对！完美通关。</div>}
                </div>
             )}
             {renderChart(log)}
          </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-3 sm:p-6 space-y-6 pb-24">
        ...
        <>
          <div className="flex flex-col gap-4 sticky top-4 z-20">
            <div className="flex flex-col sm:flex-row gap-4 items-center">
                <div className="relative flex-1 w-full"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/><input className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl shadow-sm outline-none focus:ring-2 ring-indigo-500 text-base" placeholder="搜索内容..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}/></div>
                <div className="flex gap-3 w-full sm:w-auto overflow-x-auto">
                <Button variant="outline" className="py-3 text-sm font-bold text-slate-500 border-slate-200 bg-white hover:bg-slate-50" onClick={() => setShowSettingsModal(true)}><Settings className="w-4 h-4 mr-2"/> 高级设置</Button>
                <Button variant="outline" className="py-3 text-sm font-bold text-indigo-600 border-indigo-100 bg-indigo-50 hover:bg-indigo-100" onClick={() => setShowSmartSortModal(true)}><Shuffle className="w-4 h-4 mr-2"/> 智能排序</Button>
                <Button variant="outline" className="py-3 text-sm font-bold text-slate-600 border-slate-200 bg-white hover:bg-slate-50" onClick={() => setShowShuffleConfirm(true)}><Dices className="w-4 h-4 mr-2"/> 打乱顺序</Button>
                <Button variant="ghost" className="py-3 text-sm font-bold text-red-500 hover:text-red-600 hover:bg-red-50" onClick={() => setShowResetConfirm(true)}><RotateCcw className="w-4 h-4 mr-2"/> 清空进度</Button>
                <Button variant="outline" className="py-3 text-sm font-bold" onClick={()=>{setBatchText(generateBatchText()); setShowBatchModal(true);}}><FileText className="w-4 h-4 mr-2"/> 批量编辑</Button>
                <Button className="py-3 text-sm font-bold" onClick={()=>{const n:Phrase={id:uuidv4(),english:'',chinese:'',note:'',consecutiveCorrect:0,consecutiveWrong:0,mastery:0,totalReviews:0};const newPs=[n,...phrases];setPhrases(newPs);onUpdateDeck({...deck, subject, phrases:newPs, queue:[n.id,...deck.queue]});startEditing(n);}}><Plus className="w-4 h-4 mr-2"/> 添加</Button>
                </div>
            </div>
            
            {/* Perfect Weight Summary */}
            <div className="bg-indigo-50/50 border border-indigo-100 rounded-xl p-3 flex items-center justify-between text-xs font-bold text-indigo-800">
               <div className="flex items-center gap-2"><Scale className="w-4 h-4 text-indigo-500"/> Perfect Weight Stats (Perfect phrases: {stats.perfectCount})</div>
          ...
          <div className="grid gap-2">
            {displayPhrases.map((p, index) => {
              ...
              const label = p.consecutiveCorrect > 0 ? `对${p.consecutiveCorrect}` : p.consecutiveWrong > 0 ? `错${p.consecutiveWrong}` : '新';
              const totalWrong = p.totalWrong ?? (p.totalReviews - (p.consecutiveCorrect > 0 ? p.consecutiveCorrect : 0));
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                     ...
                     <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[9px] font-black px-1.5 rounded border ${totalWrong === 0 ? 'text-emerald-400 bg-emerald-50 border-emerald-100' : 'text-rose-400 bg-rose-50 border-rose-100'}`}>Total Wrong: {totalWrong}</span>
                        </div>
                     </div>
                  </div>
                  <div className="flex items-center gap-2 pl-4 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                     ...
                     <button onClick={()=>deletePhrase(p.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4"/></button>
                  </div>
                </div>
              );
            })}
      ...
      {/* ... (history and macro tabs remain same) ... */}
      {activeTab === 'history' && (
         <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
             ...
             {filteredHistory.length === 0 ? (
                 ...
                 <div className="grid gap-3">
                     {filteredHistory.map(log => renderLogCard(log))}
                 </div>
             )}
         </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
             ...
             <div className="p-4 flex-1 overflow-hidden flex flex-col">
               <div className="text-xs text-slate-500 mb-2 p-3 bg-amber-50 text-amber-800 rounded-lg">格式：题目 | 答案 | 笔记 | 进度(正对负错) | 排序ID | 累计错误次数</div>
               <textarea className="flex-1 w-full p-4 border border-slate-200 rounded-xl font-mono text-sm leading-relaxed outline-none focus:ring-2 ring-indigo-500 resize-none" value={batchText} onChange={e=>setBatchText(e.target.value)} spellCheck={false}/>
             </div>
      ...
      {/* ... (Reset and Shuffle modals remain same) ... */}
      {showResetConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
             ...
             <p className="text-sm text-slate-500 mb-6">将重置所有词条的连对/连错计数，并恢复默认排序。此操作不可撤销。</p>
      ...
      {showShuffleConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
             ...
             <p className="text-sm text-slate-500 mb-6">将随机打乱所有词条的复习顺序。所有进度和记录将被保留。</p>
             <div className="flex flex-col gap-2">
                 ...
                 <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                     <label className="flex items-center justify-between cursor-pointer group">
                         <div className="pr-4">
                             ...
                             <div className="text-xs text-slate-400">是否将此词组本的达标速度计入全局灵根数量评级</div>
                         </div>
                 ...
                 <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                     <label className="flex items-center justify-between cursor-pointer group">
                         <div className="pr-4">
                             ...
                             <div className="text-xs text-slate-400">是否将此词组本的顿悟事件计入全局灵根质量评级</div>
                         </div>
                 ...
                 <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                     <div className="flex items-center justify-between">
                         <div className="pr-4">
                             ...
                             <div className="text-xs text-indigo-700/70">将所有的中文题目与英文答案互换</div>
                         </div>
      ...
      {showSmartSortModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto custom-scrollbar">
              <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-indigo-100 rounded-lg text-indigo-600"><Shuffle className="w-6 h-6"/></div><h3 className="text-lg font-black text-slate-800">智能排序</h3></div>
              
              <div className="mb-6">
                 <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 block">1. 筛选内容 (OR Logic)</label>
                 
                 <div className="mb-4">
                     <div className="text-[10px] font-bold text-slate-400 mb-2 uppercase">按状态 Status</div>
                     ...
                     <div className="flex flex-wrap gap-2 p-1 max-h-32 overflow-y-auto custom-scrollbar border border-slate-100 rounded-xl bg-slate-50/50">
                         {availableTags.map(tag => (
                             <button key={tag} onClick={()=>toggleTag(tag)} className={`px-2 py-1.5 rounded-lg text-xs font-black border transition-all flex items-center gap-1 ${selectedTags.has(tag) ? 'text-white shadow-sm transform scale-105 border-transparent' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-300'}`} style={selectedTags.has(tag) ? { backgroundColor: getTagColor(tag) } : {}}>
                                 {tag}
                 ...
                 <div>
                     <div className="text-[10px] font-bold text-slate-400 mb-2 uppercase">按错误历史 Total Wrong</div>
                     <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-100">
                         <span className="text-xs font-bold text-slate-500">Range:</span>
                         <input type="number" min="0" value={sortTwMin} onChange={e=>setSortTwMin(e.target.value)} className="w-16 p-1 text-center font-black border-2 border-slate-200 rounded-lg focus:border-indigo-500 outline-none text-slate-700 bg-white text-sm" placeholder="Min" />
                         <span className="text-xs font-bold text-slate-400">to</span>
                         <input type="number" min="0" value={sortTwMax} onChange={e=>setSortTwMax(e.target.value)} className="w-16 p-1 text-center font-black border-2 border-slate-200 rounded-lg focus:border-indigo-500 outline-none text-slate-700 bg-white text-sm" placeholder="Max" />
                         <div className="text-[10px] text-slate-400 ml-auto font-medium">Leave empty for any</div>
                     </div>
                 </div>
              </div>

              <div className="mb-8">
                 <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 block">2. 选择排序策略</label>
              ...
              <div className="flex gap-3">
                  ...
                  <Button fullWidth onClick={handleSmartSort} disabled={selectedTags.size === 0 && sortTwMin === '' && sortTwMax === ''}>确认排序</Button>
              </div>
           </div>
        </div>
      )}
