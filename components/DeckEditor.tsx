...
export const DeckEditor: React.FC<DeckEditorProps> = ({ deck, onUpdateDeck, onAddDecks, onBack }) => {
  ...
  const [editForm, setEditForm] = useState({ english: '', chinese: '', note: '', cooling: 0 });
  ...
  const generateBatchText = () => {
    ...
    const coolingMap = new Map<string, number>((deck.coolingPool || []).map(c => [c.id, c.wait]));
    ...
    return sortedIds.map((id, idx) => {
      ...
      const cooling = coolingMap.get(id) || 0;
      return `${p.chinese} | ${p.english} | ${note} | ${progress} | ${idx} | ${totalWrong} | ${cooling}`;
    }).filter(x => x).join('\n');
  };
  ...
  const handleResetProgress = () => {
    ...
    const updatedDeck: Deck = { ...deck, name: deckName, subject, phrases: resetPhrases, queue: orderedQueue, stats: { totalStudyTimeSeconds: 0, totalReviewCount: 0 }, sessionHistory: [], coolingPool: [] };
  ...
  const handleSmartSort = () => {
      ...
      let newQueue: string[] = [];
      let newCoolingPool = [...(deck.coolingPool || [])];

      const getCooling = (id: string) => newCoolingPool.find(c => c.id === id)?.wait;
      const setCooling = (id: string, wait: number | undefined) => {
          newCoolingPool = newCoolingPool.filter(c => c.id !== id);
          if (wait !== undefined && wait > 0) {
              newCoolingPool.push({ id, wait });
          }
      };

      if (sortStrategy === 'head') {
          newQueue = [...targetIds, ...otherIds];
          // Clear cooling for all target items (Head strategy = Active immediately)
          targetIds.forEach(id => setCooling(id, undefined));
      } else {
          const ratio = Math.max(1, interleaveRatio);
          let tIdx = 0;
          
          for (let oIdx = 0; oIdx < otherIds.length; oIdx++) {
              const otherId = otherIds[oIdx];
              newQueue.push(otherId);
              
              if ((oIdx + 1) % ratio === 0 && tIdx < targetIds.length) {
                  const targetId = targetIds[tIdx++];
                  const prevWait = getCooling(otherId); // Inherit from predecessor
                  setCooling(targetId, prevWait);
                  newQueue.push(targetId);
              }
          }
          while (tIdx < targetIds.length) { 
               const targetId = targetIds[tIdx++];
               const prevId = newQueue.length > 0 ? newQueue[newQueue.length - 1] : undefined;
               const prevWait = prevId ? getCooling(prevId) : undefined;
               setCooling(targetId, prevWait);
               newQueue.push(targetId);
          }
      }
  ...
  const confirmBatchSave = () => {
    ...
    lines.forEach((line, lineIndex) => {
      ...
      const progRaw = parts[3] || '';
      const idxRaw = parts[4] || '';
      const totalWrongRaw = parts[5] || '';
      const coolingRaw = parts[6] || '';
      
      const note = noteRaw.replace(/\\n/g, '\\n');
      const progress = parseInt(progRaw) || 0;
      const cooling = parseInt(coolingRaw) || 0;
    ...
    const newCoolingPool: {id: string, wait: number}[] = [];
    
    parsedItems.forEach(item => {
        if (item.cooling > 0) {
            newCoolingPool.push({ id: item.phrase.id, wait: item.cooling });
        }
    });

    onUpdateDeck({ ...deck, name: deckName, subject, phrases: newPhrases, queue: newQueue, coolingPool: newCoolingPool });
  ...
  const startEditing = (p: Phrase) => {
    ...
    const coolingItem = (deck.coolingPool || []).find(c => c.id === p.id);
    setEditForm({ 
        english: p.english, 
        chinese: p.chinese, 
        note: (p.note || '').replace(/\\n/g, '\n'),
        cooling: coolingItem ? coolingItem.wait : 0
    });
  };
  ...
  const savePhrase = () => {
    ...
    let newCoolingPool = [...(deck.coolingPool || [])];
    newCoolingPool = newCoolingPool.filter(c => c.id !== editingPhraseId);
    if (editingPhraseId && editForm.cooling > 0) {
        newCoolingPool.push({ id: editingPhraseId, wait: editForm.cooling });
    }

    setPhrases(updated);
    onUpdateDeck({ ...deck, name: deckName, subject, phrases: updated, coolingPool: newCoolingPool });
  ...
  const executeDelete = () => {
      ...
      const newCoolingPool = (deck.coolingPool || []).filter(c => c.id !== id);
      onUpdateDeck({ ...deck, phrases: newPhrases, queue: newQueue, coolingPool: newCoolingPool });
             ...
             <div className="flex items-center gap-4">
                 ...
                 <div>
                     ...
                     <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
                         ...
                         <span>•</span>
                         <span>{log.reviewCount} items</span>
                     </div>
                 </div>
             </div>
             <div className="text-right">
                 <div className="text-lg font-black" style={{ color: getDynamicColor(accuracy) }}>{accuracy.toFixed(0)}%</div>
                 <div className="text-[10px] text-slate-400 font-bold">{log.correctCount} / {log.wrongCount}</div>
             </div>
        </div>
                         ...
                         <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                             {log.examResults.map((res, idx) => (
                                 <div key={idx} className="flex items-center text-xs p-2 bg-white rounded-lg border border-slate-100">
                                     {res.isCorrect ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 mr-2 shrink-0"/> : <XCircle className="w-3.5 h-3.5 text-rose-500 mr-2 shrink-0"/>}
        ...
        <>
          <div className="pt-2 pb-2 -mx-2 px-2 transition-all">
            <div className="flex flex-col sm:flex-row gap-4 items-center">
                ...
                <Button variant="outline" className="py-3 text-sm font-bold text-indigo-600 border-indigo-100 bg-indigo-50 hover:bg-indigo-100 whitespace-nowrap" onClick={() => setShowSmartSortModal(true)}><Shuffle className="w-4 h-4 mr-2"/> 智能排序</Button>
                {onAddDecks && <Button variant="outline" className="py-3 text-sm font-bold text-slate-600 border-slate-200 bg-white hover:bg-slate-50 whitespace-nowrap" onClick={() => setShowSplitModal(true)}><Split className="w-4 h-4 mr-2"/> 拆分</Button>}
                <Button variant="outline" className="py-3 text-sm font-bold text-slate-600 border-slate-200 bg-white hover:bg-slate-50 whitespace-nowrap" onClick={() => setShowShuffleConfirm(true)}><Dices className="w-4 h-4 mr-2"/> 打乱</Button>
                ...
                return (
                  <div key={p.id} className="bg-white p-6 rounded-2xl shadow-lg border-2 border-indigo-500 space-y-4 animate-in zoom-in-95">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                          <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1 block">冷却轮数 Cooling Steps</label>
                          <div className="flex items-center gap-3 h-full">
                              <input type="number" min="0" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 ring-indigo-500 font-black" value={editForm.cooling} onChange={e=>setEditForm({...editForm, cooling: parseInt(e.target.value) || 0})}/>
                              <div className="text-xs text-slate-400 font-medium leading-tight">Set {'>'} 0 to place in cooling pool. Set 0 to remove.</div>
                          </div>
                      </div>
                    </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                     ...
                     <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-bold text-slate-400 bg-slate-100 px-1.5 rounded border border-slate-200 tabular-nums shrink-0" title="Word Count">{wordCount}w</span>
                            <span className={`text-[9px] font-black px-1.5 rounded border shrink-0 ${totalWrong === 0 ? 'text-emerald-400 bg-emerald-50 border-emerald-100' : 'text-rose-400 bg-rose-50 border-rose-100'}`}>Total Wrong: {totalWrong}</span>
                            {coolingItem && <span className="text-[9px] font-black px-1.5 rounded border shrink-0 text-sky-500 bg-sky-50 border-sky-100 flex items-center gap-1"><ThermometerSnowflake className="w-2.5 h-2.5"/> Cooling: {coolingItem.wait}</span>}
                        </div>
                     </div>
                  </div>
                  <div className="flex items-center gap-2 pl-4 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                     ...
                     <button onClick={()=>handleDeleteClick(p.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4"/></button>
                  </div>
                </div>
              );
            })}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-6xl flex flex-col h-[80vh]">
             ...
             <div className="p-4 flex-1 overflow-hidden flex flex-col min-h-0">
               <div className="text-xs text-slate-500 mb-2 p-3 bg-amber-50 text-amber-800 rounded-lg shrink-0">格式：题目 | 答案 | 笔记 | 进度(正对负错) | 排序ID | 累计错误次数 | 冷却轮数</div>
               <textarea className="flex-1 w-full p-4 border border-slate-200 rounded-xl font-mono text-sm leading-relaxed outline-none focus:ring-2 ring-indigo-500 resize-none overflow-y-auto" value={batchText} onChange={e=>setBatchText(e.target.value)} spellCheck={false}/>
             </div>
              ...
              <div className="space-y-4 mb-6">
                  ...
                  <label className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-white cursor-pointer hover:bg-slate-50 transition-all text-left">
                      ...
                      <div className="text-xs">
                          ...
                          <span className="block text-slate-400">保留熟练度和错误记录</span>
                      </div>
                  </label>
              </div>
