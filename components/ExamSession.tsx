...
import { ArrowLeft, CheckCircle2, XCircle, Trophy, Clock, StickyNote, Shuffle, ArrowRight } from 'lucide-react';
...
export const ExamSession: React.FC<ExamSessionProps> = ({ 
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
        ...
        if (e.code === 'Space' || e.key === 'Enter' || e.key === '1') {
            e.preventDefault();
            if (step === 'QUESTION') handleReveal();
            if (step === 'REVEAL' && (e.key === '1' || e.key === 'ArrowLeft')) handleVerdict(true);
  ...
  }, [isFinished, step, handleReveal, handleVerdict]);
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 flex flex-col h-full z-50">
       ...
       <div className="flex-1 flex flex-col items-center justify-start p-6 pt-12 overflow-y-auto custom-scrollbar">
          <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-slate-100 p-6 sm:p-8 min-h-[240px] flex flex-col items-center text-center relative overflow-hidden shrink-0">
              ...
              {step === 'REVEAL' && (
                  <div className="animate-in fade-in slide-in-from-bottom-4 space-y-4 w-full">
                      {currentPhrase.note && (
       ...
       <div className="p-4 bg-white border-t border-slate-100 shrink-0">
           <div className="max-w-xl mx-auto">
               {step === 'QUESTION' ? (
                   <Button fullWidth onClick={handleReveal} className="py-4 text-lg font-black shadow-lg">
                       显示答案 (Space)
                   </Button>
               ) : (
