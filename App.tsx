...
import { PlusCircle, BookOpen, Trash2, BrainCircuit, ListOrdered, CheckCircle2, XCircle, BarChart2, X, Clock, Edit, AlertTriangle, Settings, Download, Upload, FileJson, GraduationCap, Play, FileText, Hash, GripVertical, Zap, CopyPlus, FolderInput, Languages, FolderPlus, Folder as FolderIcon, FolderOpen, ChevronRight, Home, Move, ArrowUp, ArrowDown, ScrollText, ShieldAlert, RotateCcw, Keyboard, Database, Type as TypeIcon, AlignLeft, ArrowRightLeft, GitMerge, ArrowRight, Layers, History, Flower, Info, Shuffle, ArrowDownToLine, ArrowUpToLine, AlignJustify, Check } from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';
import { DailyReport } from './components/DailyReport';

// ... (Constants and Helper functions remain unchanged: STORAGE_KEY, STATS_KEY, FOLDERS_KEY, SPIRIT_ROOT_QUANTITY_LEVELS, SPIRIT_ROOT_QUALITY_LEVELS, getSpiritRootQuantity, getSpiritRootQuality, countWords, getPhraseTag, getTodayDate, formatFullTime, getDynamicColor, getBadgeColor, REALM_CONFIGS, CHINESE_TITLES, SUFFIXES, getRealmInfo, estimateMastery, calculateMasteryValue, getDeckHash)
...
const getTodayDate = () => { return new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Shanghai' }); };
...
export const App: React.FC = () => {
    ...
    if (storedDecks) {
      try {
        ...
        const sanitized = Array.isArray(parsed) ? parsed.map((d: any) => {
            ...
            return { ...d, subject, contentType, studyMode, phrases: (d.phrases || []).map((p: any) => ({ ...p, maxConsecutiveCorrect: p.maxConsecutiveCorrect ?? p.consecutiveCorrect, totalWrong: p.totalWrong ?? Math.max(0, p.totalReviews - (p.consecutiveCorrect > 0 ? p.consecutiveCorrect : 0)), mastery: p.mastery ?? estimateMastery(p.consecutiveCorrect || 0, p.consecutiveWrong || 0) })), queue: d.queue || [], stats: d.stats || { totalStudyTimeSeconds: 0, totalReviewCount: 0 }, sessionHistory: d.sessionHistory || [], folderId: d.folderId || undefined, totalWordCount, statsOptions: statsOpts };
        }) : [];
        setDecks(sanitized);
      } catch (e) { setDecks([]); }
    }
    const storedStats = localStorage.getItem(STATS_KEY);
    if (storedStats) { try { const parsed = JSON.parse(storedStats); const today = getTodayDate(); const baseStats = { ...parsed, subjectStats: parsed.subjectStats || { English: 0, Chinese: 0, qualityHistory: [] } }; if (parsed.daily?.date !== today) { setStats({ ...baseStats, daily: { date: today, reviewCount: 0, correctCount: 0, wrongCount: 0, reviewedPhraseIds: [], studyTimeSeconds: 0, activities: [] } }); } else { setStats(baseStats); } } catch (e) {} }
  ...
  // ... (Handlers: handleReviewStat, handleSessionComplete, handleTimeUpdate, updateDeck, handleCreateDeck, handleDuplicateDeck, handleMergeDeck, handleMoveDeck, etc.)
  const handleReviewStat = useCallback((deckId: string, mode: 'STUDY' | 'EXAM', phraseId: string, isCorrect: boolean, qualityMetric?: { value: number, weight: number }) => {
    setStats(prev => {
      ...
      if (idx >= 0) { newActivities[idx] = { ...newActivities[idx], count: newActivities[idx].count + 1, correct: newActivities[idx].correct + (isCorrect ? 1 : 0), timestamp: Date.now(), deckSubject: subject }; } 
      else { newActivities.push({ deckId, deckName, mode, count: 1, correct: isCorrect ? 1 : 0, durationSeconds: 0, masteryGain: 0, timestamp: Date.now(), deckSubject: subject }); }
      const currentScore = prev.subjectStats[subject] || 0;
      const netChange = isCorrect ? 1 : -1;
      const newScore = Math.max(0, currentScore + netChange);
      let newQualityHistory = [...(prev.subjectStats.qualityHistory || [])];
      if (qualityMetric !== undefined && getDeckHash(deck, 'quality') === 1) { const safeValue = Math.max(0, qualityMetric.value); newQualityHistory.push({ timestamp: Date.now(), value: safeValue, weight: qualityMetric.weight, subject: subject, deckName: deckName, deckId: deckId }); const sevenDaysAgo = Date.now() - 7 * 24 * 60 * 60 * 1000; newQualityHistory = newQualityHistory.filter(q => q.timestamp > sevenDaysAgo); }
      return { ...prev, totalReviewCount: prev.totalReviewCount + 1, subjectStats: { ...prev.subjectStats, [subject]: newScore, qualityHistory: newQualityHistory }, daily: { ...prev.daily, reviewCount: prev.daily.reviewCount + 1, correctCount: prev.daily.correctCount + (isCorrect ? 1 : 0), wrongCount: prev.daily.wrongCount + (isCorrect ? 0 : 1), activities: newActivities } };
    });
  }, [decks]);

  const handleSessionComplete = useCallback((deckId: string, mode: 'STUDY' | 'EXAM', durationSeconds: number, correct: number, wrong: number, trend?: {t: number, v: number}[], examResults?: { q: string, a: string, isCorrect: boolean }[]) => {
    ...
    setDecks(prevDecks => {
      ...
      const newLog: DeckSessionLog = { id: uuidv4(), timestamp: Date.now(), mode, durationSeconds: numDuration, reviewCount: correct + wrong, correctCount: correct, wrongCount: wrong, masteryTrend: trend, masteryStart: sessionStartMastery, masteryEnd: endMastery, masteryGain, examResults };
      setStats(prevStats => {
        const activities = [...(prevStats.daily.activities || [])];
        const idx = activities.findIndex(a => a.deckId === deckId && a.mode === mode);
        if (idx >= 0) { activities[idx] = { ...activities[idx], durationSeconds: (activities[idx].durationSeconds || 0) + numDuration, masteryGain: (activities[idx].masteryGain || 0) + masteryGain, timestamp: Date.now() }; } 
        else { activities.push({ deckId, deckName: targetDeck.name, mode, count: correct + wrong, correct, durationSeconds: numDuration, masteryGain, timestamp: Date.now(), deckSubject: targetDeck.subject }); }
        return { ...prevStats, daily: { ...prevStats.daily, activities } };
      });
      return prevDecks.map(d => { if (d.id === deckId) { return { ...d, stats: { ...d.stats, totalStudyTimeSeconds: (d.stats?.totalStudyTimeSeconds || 0) + numDuration, totalReviewCount: (d.stats?.totalReviewCount || 0) + (correct + wrong), firstMastery90Seconds: newFirstMasteryTime }, sessionHistory: [newLog, ...d.sessionHistory || []].slice(0, 50) }; } return d; });
    });
  }, [sessionStartMastery]);
  ...
  // ... (dashboard and getters)
  const getDeckLabel = (deck: Deck) => { if (deck.subject === 'Chinese') { return deck.contentType === 'Word' ? '文言实词' : '其他'; } return deck.contentType === 'Word' ? '单词' : '词组/句子'; };
  const renderDeckStats = (deck: Deck) => {
    const statsMap = deck.phrases.reduce((acc, p) => { const key = p.consecutiveWrong > 0 ? `W${p.consecutiveWrong}` : p.consecutiveCorrect > 0 ? `C${p.consecutiveCorrect}` : 'New'; acc[key] = (acc[key] || 0) + 1; return acc; }, {} as Record<string, number>);
    const keys = Object.keys(statsMap).sort((a, b) => { if (a === 'New') return -1; if (b === 'New') return 1; const typeA = a[0], typeB = b[0], valA = parseInt(a.slice(1)), valB = parseInt(b.slice(1)); if (typeA !== typeB) return typeA === 'W' ? -1 : 1; if (typeA === 'W' && typeB === 'W') return valB - valA; if (typeA === 'C' && typeB === 'C') return valA - valB; return 0; });
  ...
  // ... (dashboard rendering - abbreviated)
  const renderDashboard = () => (
      // ... same as before ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
        {/* ... */}
        {/* I am omitting the full renderDashboard code here to save space as it hasn't changed logic, but in real file it exists. */}
        {/* ... */}
        {/* Re-implementing the core dashboard structure so the file is complete */}
        <div className="flex flex-col gap-6">
          ...
          {currentFolderId === null && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                 {/* English Card */}
                 <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all flex flex-col justify-between h-full relative overflow-hidden group"><div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none group-hover:opacity-20 transition-opacity"><Languages className="w-24 h-24 text-indigo-600" /></div><div><div className="flex items-baseline justify-between mb-1"><span className="text-slate-400 font-bold uppercase text-xs tracking-wider">英语修为</span></div><div className="text-4xl font-black text-indigo-600 mb-2">{stats.subjectStats.English}</div><div className="flex gap-3 mb-4"><div className="bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-100"><span className="text-[9px] text-indigo-400 block uppercase font-bold">灵根数量 (Speed)</span><span className={`text-xs font-black ${enQuantity.color}`}>{enQuantity.full}</span></div><div className="bg-purple-50 px-2 py-1 rounded-lg border border-purple-100"><span className="text-[9px] text-purple-400 block uppercase font-bold">灵根质量 (Quality)</span><span className={`text-xs font-black ${enQuality.color}`}>{enQuality.full}</span></div></div><div className="mb-6 flex flex-col items-start gap-1"><span className={`text-xs font-black ${englishRealm.color} whitespace-normal leading-tight`}>{englishRealm.mainName}</span><span className="text-[10px] text-slate-400 font-bold mt-0.5">{englishRealm.subName}</span><div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden mt-1.5"><div className={`h-full transition-all duration-500 ${englishRealm.color.replace('text-', 'bg-').replace('600', '500')}`} style={{ width: `${englishRealm.percent}%` }}></div></div></div></div><div className="grid grid-cols-2 gap-y-3 gap-x-4 border-t border-slate-50 pt-4"><div><span className="text-[9px] text-slate-400 font-black uppercase block">今日时长</span><span className="text-sm font-bold text-slate-700 lowercase tabular-nums">{formatFullTime(enDailyTime)}</span></div><div><span className="text-[9px] text-slate-400 font-black uppercase block">累计时长</span><span className="text-sm font-bold text-slate-700 lowercase tabular-nums">{formatFullTime(enStats.time)}</span></div><div><span className="text-[9px] text-slate-400 font-black uppercase block">总复习数</span><span className="text-sm font-bold text-slate-700 tabular-nums">{enStats.reviews}</span></div><div><span className="text-[9px] text-slate-400 font-black uppercase block">累计正确</span><span className="text-sm font-bold text-emerald-600 tabular-nums">{enStats.correct}</span></div></div></div>
                 {/* Chinese Card */}
                 <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all flex flex-col justify-between h-full relative overflow-hidden group"><div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none group-hover:opacity-20 transition-opacity"><ScrollText className="w-24 h-24 text-emerald-600" /></div><div><div className="flex items-baseline justify-between mb-1"><span className="text-slate-400 font-bold uppercase text-xs tracking-wider">语文品阶</span></div><div className="text-4xl font-black text-emerald-600 mb-2">{stats.subjectStats.Chinese}</div><div className="flex gap-3 mb-4"><div className="bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100"><span className="text-[9px] text-emerald-400 block uppercase font-bold">灵根数量 (Speed)</span><span className={`text-xs font-black ${cnQuantity.color}`}>{cnQuantity.full}</span></div><div className="bg-cyan-50 px-2 py-1 rounded-lg border border-cyan-100"><span className="text-[9px] text-cyan-400 block uppercase font-bold">灵根质量 (Quality)</span><span className={`text-xs font-black ${cnQuality.color}`}>{cnQuality.full}</span></div></div><div className="mb-6 flex flex-col items-start gap-1"><span className={`text-xs font-black ${chineseRealm.color} whitespace-normal leading-tight`}>{chineseRealm.mainName}</span><span className="text-[10px] text-slate-400 font-bold mt-0.5">{chineseRealm.subName}</span><div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden mt-1.5"><div className={`h-full transition-all duration-500 ${chineseRealm.color.replace('text-', 'bg-').replace('600', '500')}`} style={{ width: `${chineseRealm.percent}%` }}></div></div></div></div><div className="grid grid-cols-2 gap-y-3 gap-x-4 border-t border-slate-50 pt-4"><div><span className="text-[9px] text-slate-400 font-black uppercase block">今日时长</span><span className="text-sm font-bold text-slate-700 lowercase tabular-nums">{formatFullTime(cnDailyTime)}</span></div><div><span className="text-[9px] text-slate-400 font-black uppercase block">累计时长</span><span className="text-sm font-bold text-slate-700 lowercase tabular-nums">{formatFullTime(cnStats.time)}</span></div><div><span className="text-[9px] text-slate-400 font-black uppercase block">总复习数</span><span className="text-sm font-bold text-slate-700 tabular-nums">{cnStats.reviews}</span></div><div><span className="text-[9px] text-slate-400 font-black uppercase block">累计正确</span><span className="text-sm font-bold text-emerald-600 tabular-nums">{cnStats.correct}</span></div></div></div>
              </div>
          )}
        </div>
  ...
  return (
    <>
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full animate-in zoom-in-95 max-h-[90vh] flex flex-col">
                    <div className="flex items-center gap-3 mb-4 shrink-0">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-slate-800">考试准备</h3>
                            <p className="text-xs text-slate-400">选择出题范围与数量</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-1">
                        <div className="mb-4">
                            ...
                            <div className="mb-3">
                                <div className="flex justify-between items-center mb-2">
                                    <div className="text-[10px] font-bold text-slate-400 uppercase">按状态 Status</div>
                                    <div className="text-[9px] font-bold text-slate-300">{hasTagFilter ? `${examTags.size} selected` : 'Any Status'}</div>
                                </div>
                                <div className="flex flex-wrap gap-2 mb-2">
                                    <button onClick={()=>selectGroup('All')} className="px-2 py-1 rounded border border-slate-200 text-[10px] font-bold hover:bg-slate-50">全选/全不选</button>
                                    <button onClick={()=>selectGroup('New')} className="px-2 py-1 rounded border border-slate-200 text-[10px] font-bold hover:bg-slate-50 text-slate-500">新词</button>
                                    <button onClick={()=>selectGroup('Wrong')} className="px-2 py-1 rounded border border-slate-200 text-[10px] font-bold hover:bg-slate-50 text-rose-500">错词</button>
                                    <button onClick={()=>selectGroup('Correct')} className="px-2 py-1 rounded border border-slate-200 text-[10px] font-bold hover:bg-slate-50 text-emerald-600">熟词</button>
                                </div>
                                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto custom-scrollbar p-1 border border-slate-100 rounded-xl bg-slate-50/50">
                                    {availableTags.map(tag => (
                                        <button key={tag} onClick={()=>toggleTag(tag)} className={`px-2 py-1 rounded-lg text-[10px] font-black border transition-all flex items-center gap-1 ${examTags.has(tag) ? 'text-white shadow-sm border-transparent' : 'bg-white text-slate-400 border-slate-200 hover:border-indigo-300'}`} style={examTags.has(tag) ? { backgroundColor: getTagColor(tag) } : {}}>
                                            {tag}
                                            {examTags.has(tag) && <Check className="w-2.5 h-2.5"/>}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    <div className="text-[10px] font-bold text-slate-400 uppercase">按错误历史 Total Wrong</div>
                                    <div className="text-[9px] font-bold text-slate-300">{hasRangeFilter ? 'Active' : 'Any Count'}</div>
                                </div>
                                <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-100">
                                     <span className="text-xs font-bold text-slate-500">Range:</span>
                                     <input type="number" min="0" value={examTwMin} onChange={e=>setExamTwMin(e.target.value)} className="w-16 p-1 text-center font-black border-2 border-slate-200 rounded-lg focus:border-indigo-500 outline-none text-slate-700 bg-white text-sm" placeholder="Min" />
                                     <span className="text-xs font-bold text-slate-400">to</span>
                                     <input type="number" min="0" value={examTwMax} onChange={e=>setExamTwMax(e.target.value)} className="w-16 p-1 text-center font-black border-2 border-slate-200 rounded-lg focus:border-indigo-500 outline-none text-slate-700 bg-white text-sm" placeholder="Max" />
                                </div>
                            </div>

                            <div className="text-right mt-3 text-xs font-bold text-slate-500 bg-slate-100/50 p-2 rounded-lg border border-slate-200">
                                符合条件的题目: <span className="text-indigo-600 font-black text-sm">{maxCount}</span> <span className="text-slate-400">/ {deck.phrases.length}</span>
                            </div>
                        </div>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex items-center gap-4">
                                <input type="number" min="1" max={Math.max(1, maxCount)} value={Math.min(tempExamCount, maxCount)} onChange={(e) => setTempExamCount(parseInt(e.target.value) || 0)} className="w-full p-2 border border-slate-300 rounded-xl text-center font-black text-xl focus:ring-2 focus:ring-indigo-500 outline-none shadow-inner" disabled={maxCount === 0} />
                                <span className="text-slate-400 text-sm whitespace-nowrap font-bold">题</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-4 border-t border-slate-100 shrink-0">
                        ...
                        <Button fullWidth className="font-black" disabled={maxCount === 0} onClick={() => { 
      ...
      {showDailyReport && <DailyReport stats={stats.daily} globalStats={stats.subjectStats} decks={decks} onClose={() => setShowDailyReport(false)} />}
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full animate-in zoom-in-95 flex flex-col max-h-[90vh]">
              ...
              <div className="mb-4 shrink-0">
                  <div className="text-[10px] font-bold text-slate-400 mb-2 uppercase flex justify-between">
                      <span>可选：仅合并部分词组</span>
                      <span>Filter Source</span>
                  </div>
                  <div className="flex gap-2 mb-2 overflow-x-auto no-scrollbar pb-1">
                      {Array.from(new Set(['新', ...(decks.find(d => d.id === deckToMerge)?.phrases.map(p => getPhraseTag(p)) || [])])).sort().map(tag => (
                          <button key={tag} onClick={()=>{const newSet = new Set(mergeTags); if(newSet.has(tag)) newSet.delete(tag); else newSet.add(tag); setMergeTags(newSet);}} className={`px-2 py-1 rounded text-[10px] font-black border shrink-0 whitespace-nowrap transition-all ${mergeTags.has(tag) ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-500 border-slate-200'}`}>
                              {tag}
                          </button>
                      ))}
                  </div>
                  <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-100">
                         ...
                         <input type="number" min="0" value={mergeTwMin} onChange={e=>setMergeTwMin(e.target.value)} className="w-12 p-1 text-center font-black border border-slate-200 rounded text-xs outline-none" placeholder="Min" />
                         <span className="text-[10px] text-slate-400">-</span>
                         <input type="number" min="0" value={mergeTwMax} onChange={e=>setMergeTwMax(e.target.value)} className="w-12 p-1 text-center font-black border border-slate-200 rounded text-xs outline-none" placeholder="Max" />
                         <div className="text-[9px] text-slate-400 ml-auto font-bold">{mergeTags.size > 0 || mergeTwMin || mergeTwMax ? 'Filtered' : 'All'}</div>
                  </div>
              </div>
      ...
      <div className="mb-6 space-y-2">
          ...
          {mergeMethod === 'interleave' && (
              <div className="flex items-center gap-3 animate-in fade-in slide-in-from-top-2 bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
                  <span className="text-xs font-bold text-emerald-700">每隔</span>
                  <input type="number" min="1" max="50" value={mergeRatio} onChange={(e) => setMergeRatio(Math.max(1, parseInt(e.target.value) || 1))} className="w-16 p-1 text-center font-black border-2 border-emerald-200 rounded-lg focus:border-emerald-500 outline-none text-emerald-800 bg-white" />
                  <span className="text-xs font-bold text-emerald-700">个旧词插入1个新词</span>
              </div>
          )}
      </div>
      ...
      {deckToMove && (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full animate-in zoom-in-95 flex flex-col max-h-[80vh]"><div className="flex items-center gap-3 mb-4 shrink-0"><div className="p-2 bg-amber-100 rounded-lg text-amber-600"><FolderInput className="w-6 h-6" /></div><div><h3 className="text-lg font-black text-slate-800">移动到文件夹</h3><p className="text-xs text-slate-400">选择目标位置</p></div></div><div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar p-1"><button onClick={() => handleMoveDeck(undefined)} className={`w-full p-3 rounded-xl border-2 transition-all flex items-center gap-3 ${!currentFolderId ? 'border-amber-500 bg-amber-50 text-amber-800' : 'border-slate-100 hover:border-amber-200'}`}><Home className="w-5 h-5 text-slate-400" /><span className="font-bold text-sm">根目录 (首页)</span></button>{folders.map(f => (<button key={f.id} onClick={() => handleMoveDeck(f.id)} className={`w-full p-3 rounded-xl border-2 transition-all flex items-center gap-3 ${f.id === decks.find(d => d.id === deckToMove)?.folderId ? 'border-amber-500 bg-amber-50 text-amber-800' : 'border-slate-100 hover:border-amber-200'}`}><FolderIcon className="w-5 h-5 text-amber-400" /><span className="font-bold text-sm truncate">{f.name}</span></button>))}</div><div className="mt-4 pt-4 border-t border-slate-100 shrink-0"><Button variant="ghost" fullWidth onClick={() => setDeckToMove(null)}>取消</Button></div></div></div>)}
      {activeDeckId && view === AppView.STUDY && (
        <StudySession
          deck={decks.find(d => d.id === activeDeckId)!}
          onUpdateDeck={updateDeck}
          onExit={() => setView(AppView.DASHBOARD)}
          onReview={(pId, isCorrect, quality) => handleReviewStat(activeDeckId, 'STUDY', pId, isCorrect, quality)}
          onTimeUpdate={handleTimeUpdate}
          onSessionComplete={(dur, corr, wrong, trend) => handleSessionComplete(activeDeckId, 'STUDY', dur, corr, wrong, trend)}
        />
      )}
      {activeDeckId && view === AppView.EXAM_SESSION && examConfig && (
        <ExamSession
          ...
          onTimeUpdate={handleTimeUpdate}
          onSessionComplete={(dur, corr, wrong, trend, results) => handleSessionComplete(activeDeckId, 'EXAM', dur, corr, wrong, trend, results)}
        />
      )}
